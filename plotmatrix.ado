	...
	*! Description : Plot matrices/Heat maps
	...
	v 1.21 20Apr2015  BUG -- when using lower the text doesn't appear to be lower
	v 1.22  5Jan2017  Add in user specified options on the area plots  blc and blw
	v 1.23  8May2018  Add in SE matrix to scale boxes
	*/

	prog def plotmatrix2
	version 10.1
	syntax , Mat(name) [, MATSE(name) Split(numlist) Color(string) ALLCOLORS(string) NODIAG SAVE ADDBOX(numlist) Upper Lower Distance(varname) DUnit(string) MAXTicks(integer 8) FREQ FORMATCELLS(string) BLC(string) BLW(string) *]
	...
	if "`addbox'"~="" {
	  ...
	  local txt `"`txt'|| (area boxy boxx, blw(medthin) bc(black) bfc(none) nodropb cmiss(n))"'
	}
	...
	*/
	...
	by line: gen obs2 = _n
	drop  obs
	reshape wide col1 _stack, i(obs2) j(line)  /* reshapes data into pairs */
